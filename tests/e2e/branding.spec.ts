...
test("brand roles, shared image, independent campaign snapshot and identity backup", async ({ page }) => {
  const errors: string[] = [];
  ...
  const file = { name: "logo-test.png", mimeType: "image/png", buffer: Buffer.from(png, "base64") };
  await page.getByLabel("Importer une ressource", { exact: true }).setInputFiles(file);
  ...
  await expect(page.locator(".resource-list li")).toHaveCount(5);
  await page.getByLabel("Logo — Principal", { exact: true }).selectOption({ label: "logo-test.png" });
  ...
  const brandBackup = await importZIP(backup);
  // The original logo still serves the light/dark roles, plus three fonts and the custom logo.
  expect(brandBackup.assets.size).toBe(5);
  expect(Object.keys(brandBackup.campaign.brand!.fonts)).toHaveLength(3);
  ...
  await page.getByLabel("Image du studio").selectOption({ label: "logo-test.png" });
  await page.getByRole("button", { name: "Utiliser cette image" }).click();
  await expect(page.getByRole("img", { name: "Image de la composition" })).toBeVisible();
  ...
  const saved = await importZIP(await readFile((await (await downloadEvent).path())!));
  expect(saved.assets.size).toBe(5);
  expect(saved.campaign.brand!.colors.accent).toBe("#aa33bb");
  expect(saved.campaign.assets.find((asset) => asset.source === "logo-test.png")?.credit).toBe("Auteur fictif");
  await page.getByRole("button", { name: "Branding", exact: true }).click();
  await page.getByLabel("Importer une identité ZIP").setInputFiles({ name: "branding.zip", mimeType: "application/zip", buffer: backup });
  await expect(page.getByRole("status")).toContainText("Identité importée");
  await expect(page.getByLabel("Accent", { exact: true })).toHaveValue("#aa33bb");
  await page.getByRole("button", { name: "Enregistrer l’identité", exact: true }).click();
  await expect(page.getByRole("status")).toHaveText("Identité enregistrée.");
  await page.reload();
  await expect(page.getByLabel("Nom de l’identité")).toHaveValue("Marque de recette");
  await expect(page.getByLabel("Logo — Principal", { exact: true }).locator("option:checked")).toHaveText("logo-test.png");
  await page.screenshot({ path: "test-results/branding-desktop.png", fullPage: true });
