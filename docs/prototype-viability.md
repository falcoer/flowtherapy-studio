...
The prototype validates one complete path:
...
PROTO-01 is viable when all of the following are demonstrated:
...
- direct Instagram, Facebook or YouTube publication;
- exhaustive template catalogue;
- exhaustive format support;
- advanced administration;
- brand-release workflows beyond what is required to demonstrate propagation;
...
At the end of PROTO-01, the decision is based on the observed vertical slice:

- continue toward the product roadmap if the structured model helps produce the target media;
- revise the affected model if workarounds dominate;
- stop or redesign the concept if the Studio cannot produce a usable asset without bypassing its own abstractions.
