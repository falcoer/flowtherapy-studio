...
| 0.3b | Configuration de marque | Release initiale, lot de changements, impact couleur, campagne préservée et migration explicite | En PR — première verticale exécutable |
| 0.3c | Bibliothèque | Index commun ressources/contenus, usages Marque/Campagnes/Contenus, aucune duplication de stockage | En cours — première tranche empilée sur 0.3b |
...
Voir l'[ADR 0004](docs/decisions/0004-brand-configuration-management.md), le
[modèle cible](docs/brand-configuration.md) et la [Bibliothèque](docs/library.md).
...
Brand Configuration fournit désormais le socle de releases et d'analyse d'impact.
L'incrément suivant construit Bibliothèque comme projection des ressources déjà
dédupliquées : rapprochement par SHA-256, fiche canonique, recherche et usages dans
les releases de marque, campagnes et contenus. Il ne crée aucun stockage parallèle.
Après stabilisation, Bibliothèque sera promue dans le shell global et absorbera les
accès historiques Éditorial/Médias ; Publications pourra alors devenir la prochaine
projection principale. La persistance partagée reste au jalon 0.6 ; aucune API cloud
n'est activée.
